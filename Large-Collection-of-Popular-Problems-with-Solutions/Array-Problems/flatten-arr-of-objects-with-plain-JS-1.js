...
SPECIAL NOTE - THIS SOLUTION OF GETTING A NESTED OBJECT'S PROPERTY TO BE A TOP-LEVEL PROP was NOT WORKING AT ALL INSIDE THE BACKEND ROUTING CODE OF EXPRESS in my IES Project ( /home/paul/codes-Lap/React/Volteo/IES/IES-Rohan-WIP/server/routes/importRoutes.js)

So the solution that ultimately I implemented to solve my problem is in file - flatten-arr-of-objects-with-lodash-flatMap-GREAT.js
...
const addCommodityNameFieldToItems = arr => {
  ...
  arr.map((obj, index) => {
    if (
      typeof obj.imported_commodity_objectId == "object" &&
      obj.imported_commodity_objectId !== null
    ) {
      obj.imported_commodity_name = obj.imported_commodity_objectId.name;
    }
    newArr.push(obj);
  });
  return newArr;
...
const addCommodityNameFieldToItems = arr => {
  ...
  arr.map((obj, index) => {
    if (
      ...
      obj = {
        ...obj,
        ...{ imported_commodity_name: obj.imported_commodity_objectId.name }
      };
    }
    newArr.push(obj);
  });
...
const list = addCommodityNameFieldToItems(data);
console.log(list);
